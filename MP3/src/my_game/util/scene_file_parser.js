...
class SceneFileParser {
    ...
    parseCameraJSON()   {
        let camInfo = this.xml.Camera;
        let cam = new engine.Camera(
            camInfo.Center,
            camInfo.Width,
            camInfo.Viewport 
        )
        cam.setBackgroundColor(camInfo.BgColor);
        return cam;
    }
    ...
    parseSquaresJSON(sqSet) {
        let set = this.xml.Square;
        for (var i in set)  {
            let sq = new engine.Renderable();
            sq.getXform().setPosition(set[i].Pos[0], set[i].Pos[1]);
            sq.getXform().setSize(set[i].Width, set[i].Height);
            sq.getXform().setRotationInDegree(set[i].Rotation);
            sq.setColor(set[i].Color);
